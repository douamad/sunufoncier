...
@Component({
  selector: 'jhi-evaluation-batiments',
  templateUrl: './evaluation-batiments.component.html',
})
export class EvaluationBatimentsComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.evaluationBatimentsService
      ...
      .subscribe(
        (res: HttpResponse<IEvaluationBatiments[]>) => {
  ...
  trackId(index: number, item: IEvaluationBatiments): number {
    return item.id!;
  }

  delete(evaluationBatiments: IEvaluationBatiments): void {
  ...
  protected onSuccess(data: IEvaluationBatiments[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/evaluation-batiments'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
