...
@Component({
  selector: 'jhi-evaluation-surface-batie',
  templateUrl: './evaluation-surface-batie.component.html',
})
export class EvaluationSurfaceBatieComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.evaluationSurfaceBatieService
      ...
      .subscribe(
        (res: HttpResponse<IEvaluationSurfaceBatie[]>) => {
  ...
  trackId(index: number, item: IEvaluationSurfaceBatie): number {
    return item.id!;
  }

  delete(evaluationSurfaceBatie: IEvaluationSurfaceBatie): void {
  ...
  protected onSuccess(data: IEvaluationSurfaceBatie[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/evaluation-surface-batie'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.evaluationSurfaceBaties = data ?? [];
